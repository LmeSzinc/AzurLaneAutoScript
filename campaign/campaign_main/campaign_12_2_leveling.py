...
class Campaign(CampaignBase):
    ...
    def check_s3_enemy(self):
        ...
        current = (
            self.map.select(is_enemy=True, enemy_scale=2)
        ...
        if (
            self.s3_enemy_count >= self.config.C122MediumLeveling_LargeEnemyTolerance
            and current == 0
        ):
            self.withdraw()
    ...
    def battle_0(self):
        self.check_s3_enemy()
        if self.clear_enemy(
            scale=(2,), genre=["light", "main", "treasure", "enemy", "carrier"]
        ):
            return True
        if self.clear_enemy(scale=(1,)):
            return True
        if self.clear_enemy(
            scale=(3,), genre=["light", "carrier", "enemy", "treasure", "main"]
        ):
