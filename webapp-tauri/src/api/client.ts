...
export const api = {
  ...
  updateRun: () =>
    request<{ ok: boolean }>('/update/run', { method: 'POST' }),

  remoteStatus: () =>
    request<{ alive: boolean; state: string; entry_point: string }>('/remote/status'),

  remoteStart: () =>
    request<{ ok: boolean }>('/remote/start', { method: 'POST' }),

  remoteStop: () =>
    request<{ ok: boolean }>('/remote/stop', { method: 'POST' }),
