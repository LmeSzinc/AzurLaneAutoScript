...
OCR_SOS_SIGNAL = Digit(
    OCR_SIGNAL, letter=(255, 255, 255), threshold=128, name="OCR_SOS_SIGNAL"
)
...
class CampaignSos(CampaignRun, CampaignBase):
    ...
    def _find_target_chapter(self, chapter):
        ...
        chapter_buttons = [
            button.crop(self._sos_chapter_crop) for button in all_buttons
        ]
        ocr_chapters = Digit(
            chapter_buttons,
            letter=[132, 230, 115],
            threshold=136,
            name="OCR_SOS_CHAPTER",
        )
    ...
    def run(self, name=None, folder="campaign_sos", mode="normal", total=1):
        ...
        while 1:
            ...
            if self._sos_signal_select(self.config.Sos_Chapter):
                ...
                logger.warning(
                    f"Failed to clear SOS signals, cannot locate chapter {self.config.Sos_Chapter}"
                )
                break
