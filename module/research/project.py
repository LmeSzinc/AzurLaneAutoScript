...
OCR_RESEARCH = Ocr(
    OCR_RESEARCH, name="RESEARCH", threshold=64, alphabet="0123456789BCDEGHQTMIULRF-"
)
...
def get_research_series(image):
    ...
    for button in RESEARCH_SERIES:
        im = color_similarity_2d(
            resize(crop(image, button.area), (46, 25)), color=(255, 255, 255)
        )
        ...
        if upper == lower and 1 <= upper <= 3:
            ...
            series = 0
            logger.warning(
                f"Unknown research series: button={button}, upper={upper}, lower={lower}"
            )
...
class ResearchProject:
    ...
    def __init__(self, name, series):
        ...
        for data in self.get_data(name=self.name, series=series):
            ...
            for item in data["input"]:
                result = re.search(
                    self.REGEX_INPUT, item["name"].replace(" ", "").lower()
                )
                if result:
                    self.__setattr__(f"need_{result.group(1)}", True)
            for item in data["output"]:
                result = re.search(
                    self.REGEX_SHIP, item["name"].replace(" ", "").lower()
                )
                if not self.ship:
    ...
    def get_data(self, name, series):
        ...
        for data in LIST_RESEARCH_PROJECT:
            if (data["series"] == series) and (
                data["name"].rstrip("MIRFUL-") == name.rstrip("MIRFUL-")
            ):
                yield data
...
class ResearchSelector(UI):
        ...
        while 1:
            ...
            if not self.appear(DETAIL_NEXT, offset=(20, 20)):
                ...
                self.wait_until_appear(
                    RESEARCH_COST_CHECKER, offset=(20, 20), skip_first_screenshot=True
                )
                break
    ...
    def research_sort_filter(self, enforce=False):
        ...
        FILTER.load(string)
        priority = FILTER.apply(
            self.projects, func=partial(self._research_check, enforce=enforce)
        )
    ...
    def _research_check(self, project, enforce=False):
        ...
        if project.need_cube:
            ...
            if self.config.Research_UseCube == "only_no_project" and not enforce:
                return False
            if (
                self.config.Research_UseCube == "only_05_hour"
                and not is_05
                and not enforce
            ):
                return False
        if project.need_coin:
            ...
            if self.config.Research_UseCoin == "only_no_project" and not enforce:
                return False
            if (
                self.config.Research_UseCoin == "only_05_hour"
                and not is_05
                and not enforce
            ):
                return False
        if project.need_part:
            ...
            if self.config.Research_UsePart == "only_no_project" and not enforce:
                return False
            if (
                self.config.Research_UsePart == "only_05_hour"
                and not is_05
                and not enforce
            ):
                return False
