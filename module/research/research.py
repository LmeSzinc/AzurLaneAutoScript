...
class RewardResearch(ResearchSelector):
    ...
    def research_enforce(self):
        if (not self.enforce) and (
            self.config.Research_UseCube == "only_no_project"
    ...
    def run(self):
        ...
        if self.appear(RESEARCH_FINISHED, offset=(50, 20)) or self.appear(
            RESEARCH_PENDING, offset=(50, 20)
        ):
            ...
            if research_duration_remain == 0:
                ...
                if self.research_has_finished():
                    ...
                    logger.warning(
                        "Research duration reached, but requirements not satisfied"
                    )
