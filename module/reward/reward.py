...
class Reward(UI):
    def reward_receive(self, oil, coin, exp, skip_first_screenshot=True):
        ...
        while 1:
            ...
            if (
                ...
                and self.appear_then_click(OIL, interval=60)
            ):
                confirm_timer.reset()
                click_timer.reset()
                continue
            if (
                coin
                and click_timer.reached()
                and self.appear_then_click(COIN, interval=60)
            ):
                confirm_timer.reset()
                click_timer.reset()
                continue
            if (
                exp
                and click_timer.reached()
                and self.appear_then_click(EXP, interval=60)
            ):
                confirm_timer.reset()
                click_timer.reset()
                continue
        ...
        while 1:
            ...
            for button in [MISSION_MULTI, MISSION_SINGLE]:
                if not click_timer.reached():
                    continue
                if self.appear(
                    button, offset=(0, 200), interval=interval
    ...
    def _reward_mission_all(self):
        ...
        if not self.appear(MISSION_MULTI) and not self.appear(MISSION_SINGLE):
            return False
