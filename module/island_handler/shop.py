...
class IslandShop(IslandShopUI):
    ...
    def island_shop_get_currency(self):
        for _ in self.loop(timeout=5):
            if self.ui_page_appear(page_island_shop):
                ...
                price_ocr = Digit(
                    self._island_currency_grid.buttons,
                    letter=(255, 245, 118),
                    threshold=160,
                    name="island_currency_ocr",
                )
                price = price_ocr.ocr(self.device.image)
    ...
    def island_shop_buy_ensure_amount(self, amount):
        ...
        if server.server == "cn":
            ...
            lang = "azur_lane"
        ocr = Digit(
            [ISLAND_SHOP_BUY_AMOUNT], lang=lang, letter=(57, 58, 60), threshold=160, name="island_shop_buy_amount_ocr"
        )
        ...
        self.ui_ensure_index(
            middle_integer(amount) // 10,
            one_tenth_ocr,
            ISLAND_SHOP_BUY_AMOUNT_PLUS_TEN,
            ISLAND_SHOP_BUY_AMOUNT_MINUS_TEN,
        )
    ...
    def island_shop_set_navbar_and_tab(self, recipe_id, isolated=True):
        ...
        for shop_id in search_range:
            if recipe_id in DIC_ISLAND_SHOP[shop_id]["goods"]:
                ...
                logger.info(
                    f"Recipe {recipe_id} is in shop {shop_id}, name {DIC_ISLAND_SHOP[shop_id]['name'][server.server]}"
                )
                break
