...
class IslandProductionPlanner(DaemonBase):
    ...
    SLOT_TO_GROUP = {
        ...
        9111: "nursery",
        9112: "nursery",
        9201: "manufacturing_lumber",
        9202: "manufacturing_lumber",
        9203: "manufacturing_machinery",
        9204: "manufacturing_machinery",
        9205: "manufacturing_electronic",
        9206: "manufacturing_electronic",
        9207: "manufacturing_crafts",
        9208: "manufacturing_crafts",
        9211: "fishery",
        9212: "fishery",
        9213: "fishery",
    }
    ...
    def _redirect_exchange_idle_accumulating_items(self, solution, activities):
        ...
        idle_output = {
            item_id: self.net_idle_accumulating_items.get(item_id, 0) for item_id in self.EXCHANGE_PRODUCT_IDS
        }
    ...
    def _apply_production_lp_result(self, result, problem):
        ...
        for idx, (slot, item_id) in enumerate(sale_entries, start=activity_count):
            ...
            if amount > 1e-6:
                ...
                self.daily_coin_revenue += (
                    DIC_ISLAND_ITEM[item_id]["order_price"] * (1 + self.restaurant_sales_bonus[slot]) * amount
                )
    ...
    def format_solved_production_plan(self):
        ...
        if self.sell_plan:
            for (slot, item_id), amount in sorted(self.sell_plan.items()):
                lines.append(f"slot {slot} {self._item_name(item_id)} ({item_id}): {self._format_amount(amount)}")
        else:
            lines.append("-")
        ...
        if self.group_usage_summary:
            for group, data in sorted(
                self.group_usage_summary.items(), key=lambda item: self._slot_group_sort_key(item[0])
            ):
                lines.append(
    ...
    def daily_buffer_items_to_yaml(self, use_item_name=False):
        ...
        return item_mapping_to_yaml(
            {item_id: self._get_daily_buffer_export_entry(item_id) for item_id in item_ids},
    ...
    def _build_planner_reserve_items(self, reserve_items):
        ...
        for (slot, item_id), amount in self.sell_plan.items():
            if amount <= self.NET_ACCUMULATING_EPSILON:
                continue
            reserve_items[item_id] = reserve_items.get(item_id, 0) + self.restaurant_capacity[slot]
        return {
            item_id: self._round_up_int(amount)
    ...
    def restaurant_menus_to_yaml(self):
        ...
        return {slot: item_mapping_to_yaml(menu, use_item_name=True) for slot, menu in menus.items()}
    ...
    def solve_production_plan(
        ...
        self.daily_profit_lower_limit = self.config.cross_get(
        ...
        if self.stuck_season_order_items:
            logger.info(f"Adding stuck season order {self.stuck_season_order_id} as 10-day production planner demand")
    ...
    def run(
        ...
        if tech_status_yaml is not None:
            ...
            technology_status = self.config.cross_get(
                "IslandProductionPlanner.Storage.Storage.IslandTechnologyStatus", None
            )
            if technology_status is None or self.config.cross_get(
                "IslandProductionPlanner.IslandProductionPlanner.RescanIslandTechnology", False
            ):
        ...
        if export:
            ...
            idle_accumulating_items_yaml_text = self.idle_accumulating_items_to_yaml(
                use_item_name=use_item_name_in_export
            )
            ...
            with self.config.multi_set():
                self.config.cross_set(
                    "IslandProductionPlanner.Storage.Storage.IslandTechnologyStatus", technology_status
                )
                self.config.cross_set("IslandProduction.IslandProduction.DailyBufferItems", inventory_levels_yaml_text)
                self.config.cross_set(
                    "IslandProduction.IslandProduction.IdleAccumulatingItems", idle_accumulating_items_yaml_text
                )
