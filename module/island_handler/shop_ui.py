...
class IslandShopUI(IslandUI):
    ...
    def _island_shop_side_navbar(self):
        if not self.has_shop_banner:
            return NestedNavbar(
                grids=ButtonGrid(
                    origin=(12, 80), delta=(0, 70), button_shape=(127, 70), grid_shape=(1, 7), name="ISLAND_SHOP_NAVBAR"
                ),
                subgrid_delta=(0, 58),
                subgrid_button_shape=(127, 58),
            ...
            return NestedNavbar(
                grids=ButtonGrid(
                    origin=(12, 80), delta=(0, 70), button_shape=(127, 70), grid_shape=(1, 8), name="ISLAND_SHOP_NAVBAR"
                ),
                subgrid_delta=(0, 58),
                subgrid_button_shape=(127, 58),
    ...
    def island_shop_side_navbar_ensure(self, main_index, sub_index=None, skip_first_screenshot=True):
        if not self.has_shop_banner:
            return self._island_shop_side_navbar.set(
                self, main_index=main_index, sub_index=sub_index, skip_first_screenshot=skip_first_screenshot
            )
        else:
            return self._island_shop_side_navbar.set(
                self, main_index=main_index + 1, sub_index=sub_index, skip_first_screenshot=skip_first_screenshot
            )
