...
class OperationSiren(OSMap):
    ...
    def os_finish_daily_mission(self):
        ...
        while 1:
            ...
            self.os_order_execute(
                ...
                and result != "pinned_at_archive_zone",
            )
            self.run_auto_search()
            self.handle_after_auto_search()
    ...
    def os_meowfficer_farming(self):
        ...
        while 1:
            ...
            if (
                self.config.OpsiAshBeacon_AshAttack
            ...
            if self.config.OpsiMeowfficerFarming_TargetZone != 0:
                try:
                    ...
                    self.fleet_set(self.config.OpsiFleet_Fleet)
                    self.os_order_execute(
                        recon_scan=False, submarine_call=self.config.OpsiFleet_Submarine
                    )
                    self.run_auto_search()
    ...
    def _os_explore_task_delay(self):
        ...
        self.config.modified[
            "OpsiMeowfficerFarming.OpsiMeowfficerFarming.ActionPointPreserve"
        ] = preserve
    ...
    def _os_explore(self):
        ...
        def end():
            ...
            logger.info(
                "To run again, clear OpsiExplore.Scheduler.NextRun and set OpsiExplore.OpsiExplore.LastZone=0"
            )
        ...
        try:
            ...
            logger.warning(
                f"Invalid OpsiExplore_LastZone={self.config.OpsiExplore_LastZone}, re-explore"
            )
            last_zone = 0
        ...
        if last_zone in order:
            ...
            logger.info(
                f"Last zone: {self.name_to_zone(last_zone)}, next zone: {order[:1]}"
            )
        elif last_zone == 0:
    ...
    def clear_obscure(self):
        ...
        self.fleet_set(self.config.OpsiFleet_Fleet)
        self.os_order_execute(
            recon_scan=True, submarine_call=self.config.OpsiFleet_Submarine
        )
    ...
    def clear_abyssal(self):
        ...
        self.config.override(
            OpsiGeneral_DoRandomMapEvent=False, HOMO_EDGE_DETECT=False, STORY_OPTION=0
        )
        self.zone_init()
