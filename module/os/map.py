...
class OSMap(OSFleet, Map, GlobeCamera, StrategicSearchHandler):
    ...
    def cl1_ap_preserve(self):
        ...
        """
        if self.is_cl1_enabled and get_os_reset_remain() > 2 and self.get_yellow_coins() > self.yellow_coins_preserve:
    ...
    def os_auto_search_daemon(self, drop=None, strategic=False):
        ...
        for _ in self.loop():
            ...
            if not unlock_checked:
                if self.appear(AUTO_SEARCH_OS_MAP_OPTION_OFF, offset=(5, 120)) or self.appear(AUTO_SEARCH_OS_MAP_OPTION_OFF_DISABLED, offset=(5, 120)) or self.appear(AUTO_SEARCH_OS_MAP_OPTION_ON, offset=(5, 120)):
                    unlock_checked = True
    ...
    def map_rescan_current(self, drop=None):
        ...
        if "is_exploration_container" not in self._solved_map_event and grids and grids[0].is_exploration_container:
            grid = grids[0]
