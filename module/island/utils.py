...
def item_mapping_to_yaml(items, use_item_name=False):
    payload = {item_export_key(item_id, use_item_name=use_item_name): value for item_id, value in sorted(items.items())}
...
def normalize_technology_status(technology_status=None):
    ...
    return {int(technology_id): bool(unlocked) for technology_id, unlocked in technology_status.items()}
...
def parse_item_need_deadlines(item_need, default_period=1):
    if not item_need:
        return []
    if (
        isinstance(item_need, dict)
        and item_need.get("deadlines")
        and ("total_need_count" in item_need or "rate_per_day" in item_need)
    ):
    ...
    if isinstance(item_need, (list, tuple)) or (isinstance(item_need, dict) and "deadlines" in item_need):
        ...
        return [(deadline["count"], deadline["period"]) for deadline in data.get("deadlines", [])]
