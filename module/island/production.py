...
class IslandProduction(IslandRecipe, IslandDock):
    ...
    def is_slot_finished(self, slot_button: Button):
        ...
        return self.image_color_count(tick_button, (255, 255, 255), threshold=240, count=70)
    ...
    def claim_slot_reward(self, slot_button: Button):
        ...
        for _ in self.loop():
            ...
            if self.ui_page_appear(page_island_manage, interval=2):
                ...
                continue
        click_timer = Timer(1, count=2).reset()
        for _ in self.loop(timeout=7):
            ...
            logger.warning(f'Failed to claim reward for slot {slot_button}')
        click_timer = Timer(1, count=2).reset()
        for _ in self.loop():
            if not self.is_enter_window_shown():
                return True
    ...
    def dispatch_slot(self, slot_id, slot_button, worker='manjuu'):
        ...
        for _ in self.loop():
            if self.is_in_island_dock():
                break
            if self.appear_then_click(ISLAND_PRODUCTION_SELECT_CHARACTER, offset=(60, 20), interval=2):
                continue
        if worker != 'manjuu':
    ...
    def ensure_island_production_page(self):
        ...
        for _ in self.loop(timeout=10):
            if self.ui_page_appear(page_island_manage, offset=(0, 20)):
                break
        self.island_manage_side_navbar_ensure(upper=1)
