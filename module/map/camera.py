...
class Camera(MapOperation):
    ...
    def _map_swipe(self, vector, box=(123, 159, 1175, 628)):
        ...
        if np.any(np.abs(vector) > self.config.MAP_SWIPE_DROP):
            ...
            if self.config.DEVICE_CONTROL_METHOD == "minitouch":
                distance = (
                    self.view.swipe_base * self.config.MAP_SWIPE_MULTIPLY_MINITOUCH
                )
            else:
    ...
    def _update(self, camera=True):
        ...
        self._view_init()
        try:
            if not self.is_in_map() and not self.is_in_strategy_submarine_move():
            ...
            if self.info_bar_count():
                ...
                if hasattr(self, "os_auto_search_quit"):
                    ...
                    return False
            elif not self.is_in_map() and not self.is_in_strategy_submarine_move():
    ...
    def ensure_edge_insight(
        self, reverse=False, preset=None, swipe_limit=(3, 2), skip_first_update=True
    ):
        """
        ...
        x_swipe, y_swipe = np.multiply(
            swipe_limit, random_direction(self.config.MAP_ENSURE_EDGE_INSIGHT_CORNER)
        )
    ...
    def get_swipe_area_opt(self, map_vector):
        ...
        def globe_to_local(globe_grids):
            ...
            for globe in globe_grids:
                location = tuple(
                    np.array(globe.location) - self.camera + self.view.center_loca
                )
        ...
        blacklist = [grid.outer for grid in blacklist] + local_to_area(
            blacklist, pad=-5
        )
