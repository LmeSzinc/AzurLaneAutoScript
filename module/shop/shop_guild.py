...
class GuildShop(ShopClerk, ShopUI, ShopStatus):
    ...
    def cost_template_folder(self):
        if self.config.SERVER in ['cn', 'en', 'jp']:
            return './assets/shop/guild_white/cost'
        elif self.config.SERVER in ['tw']:
    ...
    def shop_guild_items(self):
        ...
        if self.config.SERVER in ['cn', 'en', 'jp']:
            shop_guild_items = ShopItemGrid(shop_grid, templates={}, template_area=(0, 0, 63, 63), 
                                            amount_area=(31, 52, 68, 66), cost_area=(0, 141, 36, 161),
                                            price_area=(20, 139, 80, 162))
        elif self.config.SERVER in ['tw']:
