...
class GeneralShop(ShopClerk, ShopUI, ShopStatus):
    ...
    def cost_template_folder(self):
        if self.config.SERVER in ['cn', 'en', 'jp']:
            return './assets/shop/general_white/cost'
        elif self.config.SERVER in ['tw']:
    ...
    def shop_general_items(self):
        ...
        if self.config.SERVER in ['cn', 'en', 'jp']:
            shop_general_items = ShopItemGrid(shop_grid, templates={}, template_area=(0, 0, 63, 63), 
                                            amount_area=(31, 52, 68, 66), cost_area=(0, 140, 37, 160),
