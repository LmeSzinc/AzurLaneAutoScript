    ...
    def run(self):
        ...
        if self.config.Benchmark_Uiautomator2Screenshot:
            data.append(
                [
                    "uiautomator2",
                    self.benchmark_test(self.device.screenshot_uiautomator2),
                ]
            )
        if self.config.Benchmark_AscreencapScreenshot:
            data.append(
                ["aScreenCap", self.benchmark_test(self.device.screenshot_ascreencap)]
            )
        if self.config.Benchmark_AscreencapncScreenshot:
            data.append(
                [
                    "aScreenCap_nc",
                    self.benchmark_test(self.device.screenshot_ascreencap_nc),
                ]
            )
        screenshot = data
        ...
        if self.config.Benchmark_Uiautomator2Click:
            ...
            data.append(
                [
                    "uiautomator2",
                    self.benchmark_test(self.device.click_uiautomator2, x, y),
                ]
            )
        if self.config.Benchmark_MinitouchClick:
            x, y = random_rectangle_point(area)
            data.append(
                ["minitouch", self.benchmark_test(self.device.click_minitouch, x, y)]
            )
