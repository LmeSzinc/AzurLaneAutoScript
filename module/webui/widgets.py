...
class ScrollableCode:
    ...
    """
    def __init__(self, keep_buttom=True) -> None:
        self.keep_buttom = keep_buttom
...
def put_textarea_(name, title, help=None, value='', readonly=None):
    return output(
        put_column([
            put_text(title).style("arg-title"),
            put_text(help).style("arg-help"),
            put_textarea(name, value=value, readonly=readonly, code={"theme": "idea", "lineWrapping": True, "lineNumbers": False})
        ], size="auto auto auto")
    ).style("container-args") if help \
    else output(
        put_column([
...
def put_checkbox_(name, title, help=None, value=False, width="12rem"):
    ...
    return output(
        put_row([
            ...
            put_checkbox(name, options=[{'label': '', 'value': True, 'selected': value}]).style("text-align: center")
        ], size=f"1fr {width}")
