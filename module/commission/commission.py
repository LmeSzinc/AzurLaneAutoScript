...
from module.commission.project import COMMISSION_FILTER, SHORTEST_FILTER, Commission
...
COMMISSION_SCROLL = Scroll(
    COMMISSION_SCROLL_AREA, color=(247, 211, 66), name="COMMISSION_SCROLL"
)
...
class RewardCommission(UI, InfoHandler):
    ...
    def _commission_check(self, commission):
        ...
        if not commission.valid or commission.status != "pending":
            return False
        if (
            not self.config.Commission_DoMajorCommission
            and commission.category_str == "major"
        ):
            return False
        ...
        while 1:
            ...
            if self.appear(COMMISSION_ADVICE, offset=(5, 20), interval=7):
                area = (
                    0,
                    0,
                    image_size(self.device.image)[0],
