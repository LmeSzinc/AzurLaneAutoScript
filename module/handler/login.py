...
class LoginHandler(Combat):
    def _handle_app_login(self):
        ...
        while 1:
            ...
            if self.appear_then_click(
                MAINTENANCE_ANNOUNCE, offset=(30, 30), interval=5
            ):
                continue
    ...
    def ensure_no_unfinished_campaign(self, confirm_wait=3):
        ...
        self.ui_click(
            MAIN_GOTO_CAMPAIGN,
            check_button=in_campaign,
            additional=ensure_campaign_retreat,
    ...
    def handle_user_agreement(self, xp, hierarchy):
        ...
        if server.server == "cn":
            ...
            logger.info(
                "user agreement position find result: "
                + ", ".join("%.2f" % _ for _ in start_pos)
            )
            logger.info(
                "user agreement area expect:          " + "x:963-973, y:259-279"
            )
    ...
    def handle_user_login(self, xp, hierarchy) -> bool:
        ...
        if login_wait_results is False:
            ...
            USER_LOGIN_BTN = Button(
                area=login_wait_results,
                color=(),
                button=login_wait_results,
                name="USER_LOGIN_BTN",
            )
