...
class StrategyHandler(InfoHandler):
    ...
    def strategy_set_execute(self, formation_index=None, sub_view=None, sub_hunt=None):
        ...
        """
        logger.info(
            f"Strategy set: formation={formation_index}, submarine_view={sub_view}, submarine_hunt={sub_hunt}"
        )
    ...
    def handle_strategy(self, index):
        ...
        if self.__getattribute__(f"fleet_{index}_formation_fixed"):
            return False
        expected_formation = self.config.__getattribute__(
            f"Fleet_Fleet{index}Formation"
        )
        if (
            self._strategy_get_from_map_buff() == expected_formation
            and not self.config.Submarine_Fleet
        ):
        ...
        self.strategy_set_execute(
            ...
            sub_view=False,
            sub_hunt=bool(self.config.Submarine_Fleet)
            and self.config.Submarine_Mode == "hunt_only",
        )
        self.strategy_close()
        self.__setattr__(f"fleet_{index}_formation_fixed", True)
        return True
    ...
    def strategy_submarine_move_confirm(self):
        ...
        while 1:
            if self.appear_then_click(
                SUBMARINE_MOVE_CONFIRM, offset=(20, 20), interval=5
            ):
                pass
    ...
    def strategy_submarine_move_cancel(self):
        ...
        while 1:
            if self.appear_then_click(
                SUBMARINE_MOVE_CANCEL, offset=(20, 20), interval=5
            ):
                pass
            if self.handle_popup_confirm("SUBMARINE_MOVE"):
                pass
