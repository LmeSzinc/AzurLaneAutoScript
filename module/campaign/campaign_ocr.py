...
class CampaignOcr(ModuleBase):
    ...
    def campaign_extract_name_image(self, image):
        ...
        if "blue" in self.config.STAGE_ENTRANCE:
            digits += self.campaign_match_multi(
                ...
                image,
                extract_letters(
                    self._stage_image, letter=(255, 255, 255), threshold=153
                ),
                name_offset=(55, 0),
                name_size=(60, 16),
            )
            digits += self.campaign_match_multi(
                ...
                image,
                extract_letters(
                    self._stage_image, letter=(99, 223, 239), threshold=153
                ),
                name_offset=(60, 12),
                name_size=(60, 16),
            )
    ...
    def _extract_stage_name(image):
        ...
        interval = 5
        x_color = (
            np.convolve(np.mean(image, axis=0), np.ones(interval), "valid") / interval
        )
