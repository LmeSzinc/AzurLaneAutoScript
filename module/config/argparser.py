...
def main(ini_name=''):
    ...
    sos = sos_parser.add_argument_group(
        '潜艇图设置', '本模块在无出战, 通用页内才可启动;\n启动后会点击海域救援列表末(可选#1)的信号扫描, 并跳转按以下阵型出战自动寻敌,\n循环直到扫描次数耗尽, 则转入收菜', gooey_options={'label_color': '#931D03'})
    sos.add_argument('--潜艇图队伍', default=default('--潜艇图队伍'), help='设置潜艇图的队伍, 顺序: 一队二队潜艇队\n例如 "4, 6", "4, 0", "4, 6, 1"\n填0跳过不打', gooey_options={'label_color': '#4B5F83'})
    sos.add_argument('--可选#1_不是打末章而是', default=default('--可选#1_不是打末章而是'), help='默认为x=0(扫主线最后一章),\n主线未通第10章建议选x=1\n可设置扫描信号时选\n(x=1)倒数第二章 或 (x=2)倒数第三章\n注意: 不会理会其他位置已经扫描出的信号', choices=['0', '1', '2'], gooey_options={'label_color': '#4B5F83'})
