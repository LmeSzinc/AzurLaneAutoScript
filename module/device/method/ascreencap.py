...
class AScreenCap(Connection):
    ...
    def ascreencap_init(self):
        ...
        filepath = os.path.join(
            self.config.ASCREENCAP_FILEPATH_LOCAL, arc, "ascreencap"
        )
    ...
    def __uncompress(self, screenshot):
        ...
        compressed_data_header = np.frombuffer(
            raw_compressed_data[0:20], dtype=np.uint32
        )
        ...
        data = lz4.block.decompress(
            raw_compressed_data[20:], uncompressed_size=uncompressed_size
        )

        image = np.frombuffer(data, dtype=np.uint8)
        # Equivalent to cv2.imdecode()
        shape = image.shape[0]
        image = image[shape - width * height * channel :].reshape(
    ...
    def screenshot_ascreencap(self):
        stream = self.adb_shell(
            [self.config.ASCREENCAP_FILEPATH_REMOTE, "--pack", "2", "--stdout"],
            stream=True,
        )
