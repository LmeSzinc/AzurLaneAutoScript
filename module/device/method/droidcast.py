...
class DroidCast(Uiautomator2):
    ...
    def droidcast_url(self, url='/preview', format='png', quality=100):
        if self.is_mumu_over_version_356:
            w, h = self.droidcast_width, self.droidcast_height
            if self.orientation == 0:
                return f'http://127.0.0.1:{self._droidcast_port}{url}?width={w}&height={h}&format={format}&quality={quality}'
            elif self.orientation == 1:
                return f'http://127.0.0.1:{self._droidcast_port}{url}?width={h}&height={w}&format={format}&quality={quality}'
            else:
                # logger.warning('DroidCast receives invalid device orientation')
                pass

        return f'http://127.0.0.1:{self._droidcast_port}{url}?format={format}&quality={quality}'
    ...
    def droidcast_init(self):
        ...
        if self.config.DROIDCAST_VERSION == 'DroidCast':
            ...
            logger.attr('DroidCast_raw', self.droidcast_raw_url(compress='lz4'))
            self.droidcast_wait_startup()
        else:
    ...
    @retry
    def screenshot_droidcast(self, format='png', quality=100):
        self.config.DROIDCAST_VERSION = 'DroidCast'
        if format != 'png':
            self.config.DROIDCAST_VERSION += '_' + format
        if quality != 100:
            self.config.DROIDCAST_VERSION += f'_{quality}'
        if self.is_mumu_over_version_356:
            if not self.droidcast_width or not self.droidcast_height:
                self._droidcast_update_resolution()

        resp = self.droidcast_session.get(self.droidcast_url(format=format, quality=quality), timeout=3)
    ...
    def screenshot_droidcast_webp(self):
        return self.screenshot_droidcast(format='webp')

    def screenshot_droidcast_jpeg(self):
        return self.screenshot_droidcast(format='jpeg')

    def screenshot_droidcast_jpeg_98(self):
        return self.screenshot_droidcast(format='jpeg', quality=98)

    def screenshot_droidcast_jpeg_96(self):
        return self.screenshot_droidcast(format='jpeg', quality=96)
