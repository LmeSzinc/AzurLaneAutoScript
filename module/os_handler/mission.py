...
class MissionHandler(GlobeOperation, ZoneManager):
    ...
    def os_mission_enter(self, skip_first_screenshot=True):
        ...
        while 1:
            ...
            # End
            if (
                self.appear(MISSION_CHECK, offset=(20, 20))
                and not self.appear(MISSION_FINISH, offset=(20, 20))
                and not (
                    self.appear(MISSION_CHECKOUT, offset=(20, 20))
                ...
                if confirm_timer.reached():
                    ...
                    break
            elif self.appear(MISSION_CHECK, offset=(20, 20)) and (
                self.appear(MISSION_CHECKOUT, offset=(20, 20))
                and MISSION_CHECKOUT.match_appear_on(self.device.image)
            ):
                # Found one mission.
    ...
    def os_get_next_mission(self):
        ...
        if not (
            self.appear(MISSION_CHECKOUT, offset=checkout_offset)
            and MISSION_CHECKOUT.match_appear_on(self.device.image)
        ):
        ...
        if self.is_in_opsi_explore():
            logger.info(
                "OpsiExplore is under scheduling, accept missions and receive rewards only"
            )
            self.os_mission_quit()
            return False
        ...
        while 1:
            ...
            if self.appear_then_click(
                MISSION_CHECKOUT, offset=checkout_offset, interval=2
            ):
                continue
    ...
    def os_mission_overview_accept(self):
        ...
        while 1:
            ...
            if self.info_bar_count():
                ...
                break
            if self.appear_then_click(
                MISSION_OVERVIEW_ACCEPT, offset=(20, 20), interval=0.2
            ):
                confirm_timer.reset()
                continue
            else:
                ...
                if confirm_timer.reached():
                    ...
                    break
            if self.appear_then_click(
                MISSION_OVERVIEW_ACCEPT_SINGLE, offset=(20, 20), interval=0.2
            ):
                confirm_timer.reset()
                continue
        ...
        # is_in_globe
        self.ui_back(
            appear_button=MISSION_OVERVIEW_CHECK,
    ...
    def is_in_opsi_explore(self):
        ...
        """
        enable = deep_get(
            self.config.data, keys="OpsiExplore.Scheduler.Enable", default=False
        )
        next_run = deep_get(
            self.config.data, keys="OpsiExplore.Scheduler.NextRun", default=DEFAULT_TIME
        )
