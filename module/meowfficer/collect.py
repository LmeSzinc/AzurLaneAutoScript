...
SWITCH_LOCK.add_status(
    "lock", check_button=MEOWFFICER_APPLY_UNLOCK, click_button=MEOWFFICER_APPLY_LOCK
)
SWITCH_LOCK.add_status(
    "unlock", check_button=MEOWFFICER_APPLY_LOCK, click_button=MEOWFFICER_APPLY_UNLOCK
)
...
class MeowfficerCollect(MeowfficerBase):
    def _meow_detect_shift(self, skip_first_screenshot=True):
        ...
        while 1:
            ...
            if self.image_color_count(
                MEOWFFICER_SHIFT_DETECT,
                color=MEOWFFICER_SHIFT_DETECT.color,
                threshold=221,
                count=650,
            ):
                if not flag:
    ...
    def _get_meow_talent_grid(self) -> Tuple[List[Button], bool]:
        ...
        logger.info(
            "Configured to retain this type of meowfficer, "
        ...
        grid = (
            MEOWFFICER_TALENT_GRID_2
            if self._meow_detect_shift()
            else MEOWFFICER_TALENT_GRID_1
        )

        for btn in grid.buttons:
            # Empty slot; check for many white pixels
            if self.image_color_count(
                btn, color=(255, 255, 247), threshold=221, count=200
            ):
                continue
            ...
            # i.e. roman numerals
            if self.image_color_count(
                btn, color=(255, 255, 255), threshold=221, count=25
            ):
        ...
        logger.info(
            "At least one special talent ability detected"
        ) if special_talent else logger.info("No special talent abilities detected")
    ...
    def _meow_talent_cap_handle(self, talent_button: List[Button], drop):
        for btn in talent_button:
            self.ui_click(
                btn,
                check_button=MEOWFFICER_TALENT_CLOSE,
                appear_button=MEOWFFICER_GET_CHECK,
                skip_first_screenshot=True,
            )
            drop.add(self.device.image)
            self.ui_click(
                MEOWFFICER_TALENT_CLOSE,
                check_button=self._check_popup_exit,
                appear_button=MEOWFFICER_TALENT_CLOSE,
                skip_first_screenshot=True,
            )
    ...
    def _check_popup_exit(self):
        ...
        """
        if self.appear(
            MEOWFFICER_GET_CHECK, offset=(40, 40)
        ) and MEOWFFICER_GET_CHECK.match_appear_on(self.device.image):
            return True
    ...
    def _meow_skip_lock(self):
        ...
        self.ui_click(
            MEOWFFICER_TRAIN_CLICK_SAFE_AREA,
            appear_button=MEOWFFICER_GET_CHECK,
            check_button=MEOWFFICER_CONFIRM,
            offset=(40, 40),
            retry_wait=3,
            skip_first_screenshot=True,
        )
            ...
            if self.appear(MEOWFFICER_GET_CHECK, offset=(40, 40), interval=3):
                ...
                with self.stat.new(
                    ...
                    if self.appear(MEOWFFICER_GOLD_CHECK, offset=(40, 40)):
                        if (
                            not self.config.MeowfficerTrain_RetainTalentedGold
                            or not special_talent
                        ):
                    ...
                    if self.appear(MEOWFFICER_PURPLE_CHECK, offset=(40, 40)):
                        if (
                            self.config.MeowfficerTrain_RetainTalentedPurple
                            and special_talent
                        ):
                            self._meow_apply_lock()
    ...
    def meow_collect(self, collect_all=True):
        ...
        if self.appear(MEOWFFICER_TRAIN_COMPLETE, offset=(20, 20)):
            ...
            self.ui_click(
                button,
                check_button=MEOWFFICER_GET_CHECK,
