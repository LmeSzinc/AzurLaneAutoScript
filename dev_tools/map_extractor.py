...
class MapData:
    ...
    def __init__(self, data, data_loop):
        ...
        try:
            ...
            if self.data_loop is not None:
                ...
                if all(
                    [
                        d1 == d2
                        for d1, d2 in zip(
                            self.map_data.values(), self.map_data_loop.values()
                        )
                    ]
                ):
            ...
            self.MAP_HAS_MYSTERY = (
                sum([b.get("mystery", 0) for b in self.spawn_data]) > 0
            )
    ...
    def parse_spawn_data(data):
        ...
        if (
            "".join([str(item) for item in data["elite_refresh"].values()]) != "100"
    ...
    def get_file_lines(self, has_modified_campaign_base):
        ...
        camera_data = camera_2d(
            get_map_active_area(self.map_data), sight=(-3, -1, 3, 2)
        )
        lines.append(
            f"MAP.camera_data = {[location2node(loca) for loca in camera_data]}"
        )
        camera_sp = camera_spawn_point(
            camera_data, sp_list=[k for k, v in self.map_data.items() if v == "SP"]
        )
        lines.append(
        ...
        for y in range(self.shape[1] + 1):
            lines.append(
                "    "
                + " ".join([self.map_data[(x, y)] for x in range(self.shape[0] + 1)])
            )
        lines.append('"""')
        if self.map_data_loop is not None:
            ...
            for y in range(self.shape[1] + 1):
                lines.append(
                    "    "
                    + " ".join(
                        [self.map_data_loop[(x, y)] for x in range(self.shape[0] + 1)]
                    )
                )
            lines.append('"""')
    ...
    def write(self, path):
        ...
        has_modified_campaign_base = os.path.exists(
            os.path.join(path, "campaign_base.py")
        )
        ...
        with open(file, "w") as f:
            for text in self.get_file_lines(
                has_modified_campaign_base=has_modified_campaign_base
            ):
                f.write(text + "\n")
...
class ChapterTemplate:
    ...
    def extract(self, maps, folder):
        ...
        print("<<< CONFIRM >>>")
        print(
            "Please confirm selected the correct maps before extracting.\n"
