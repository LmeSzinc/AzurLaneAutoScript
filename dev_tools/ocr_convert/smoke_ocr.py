...
def main():
    ...
    model = AlOcr(
        model_name="azur_lane", root=os.path.join(REPO_ROOT, "bin", "cnocr_models", "azur_lane"), name="azur_lane"
    )
    ...
    for i, w in enumerate((80, 200, 120)):
        cv2.putText(images[i], "123456"[: max(1, w // 25)], (5, 24), cv2.FONT_HERSHEY_SIMPLEX, 0.8, 0, 2)
    start = time.time()
    ...
    model_cn = AlOcr(model_name="cnocr", root=os.path.join(REPO_ROOT, "bin", "cnocr_models", "cnocr"), name="cnocr")
