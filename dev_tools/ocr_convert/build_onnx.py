...
class Builder:
    ...
    def _convert_Activation(self, idx, node):
        ...
        self.onnx_nodes.append(
            helper.make_node(op, [self.resolve_input(node["inputs"][0])], [self.value(idx, 0)], name=node["name"])
        )
    ...
    def _convert_Pooling(self, idx, node):
        ...
        if any(pad):
            kw["pads"] = list(pad) * 2
        self.onnx_nodes.append(helper.make_node(op, [data], [self.value(idx, 0)], name=node["name"], **kw))

    def _convert_Concat(self, idx, node):
        inputs = [self.resolve_input(i) for i in node["inputs"]]
        self.onnx_nodes.append(
            helper.make_node("Concat", inputs, [self.value(idx, 0)], name=node["name"], axis=int(node["attrs"]["dim"]))
        )
    ...
    def _convert_Reshape(self, idx, node):
        ...
        self.onnx_nodes.append(
            helper.make_node(
                "Reshape",
                [self.resolve_input(node["inputs"][0]), f"{node['name']}_shape"],
                [self.value(idx, 0)],
                name=node["name"],
            )
        )
    ...
    def _convert_FullyConnected(self, idx, node):
        ...
        self.onnx_nodes.append(
            helper.make_node(
                "Gemm", [data, w, b], [self.value(idx, 0)], name=node["name"], alpha=1.0, beta=1.0, transB=1
            )
        )
    ...
    def _emit_gru_scan(self, rnn_idx, rnn_node, data_val):
        ...
        for tag, x_t, h_t, h_new in (("f", "x_fwd_t", "h_fwd", h_fwd_new), ("b", "x_bwd_t", "h_bwd", h_bwd_new)):
            ...
            body_nodes += [
                helper.make_node(
                    "Constant", [], [out.format("one")], value=helper.make_tensor("one", TensorProto.FLOAT, [1], [1.0])
                ),
        ...
        self.onnx_nodes += [
            ...
            helper.make_node(
                "Slice", [f"{name}_full_shape", f"{name}_slice_starts", f"{name}_slice_ends"], [f"{name}_n_vec"]
            ),
            helper.make_node(
                "ConstantOfShape",
                [f"{name}_n_vec"],
            ...
            helper.make_node(
                "ReverseSequence", [f"{name}_yb", seq_lens], [f"{name}_yb_rev"], batch_axis=1, time_axis=0
            ),
        ]
        self.onnx_nodes.append(
            helper.make_node(
                "Concat", [f"{name}_yf", f"{name}_yb_rev"], [self.value(rnn_idx, 0)], name=f"{name}_concat", axis=2
            )
        )
            ...
            if name in FOLDED_NODES:
                if name == "densenet0_reshape0":
                    ...
                    self.onnx_nodes += [
                        ...
                        helper.make_node(
                            "Transpose", [t2], [self.value(self.nodes_by_name["crnn0_transpose0"])], perm=[1, 0, 2]
                        ),
                    ]
                continue
            if idx == rnn_idx:
                # GRU via Scan, emitted in topological position
                self._emit_gru_scan(rnn_idx, node, self.value(self.nodes_by_name["crnn0_transpose0"]))
                continue
