...
class RelativeRecord:
    ...
    def count(self, area):
        ...
        while np.sum(mask) < self.images_amount and count < MAX_FRAME:
            ...
            mask_inv = mask == False
            m = [
                np.max(cv2.matchTemplate(image, template, cv2.TM_CCOEFF_NORMED))
                > THRESHOLD
    ...
    def get_gif(self, area):
        ...
        for n, image in enumerate(self.images):
            ...
            templates.append(
                crop(image, area=area_offset(area, np.subtract(max_loca, area[:2])))
            )
    ...
    def run_by_size(self, size):
        ...
        print("Saving gif")
        for count, sim, templates in zip(
            sim_dict.keys(), sim_dict.values(), template_dict.values()
        ):
