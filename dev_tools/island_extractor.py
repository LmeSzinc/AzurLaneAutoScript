        ...
        for _index, season in self.season.items():
            for activity_id, activity in activity_dict.items():
                if (
                    ...
                    and season["end_time"]["cn"] is not None
                    and activity["start_time"]["cn"] is not None
                    and activity["end_time"]["cn"] is not None
                ):
                    if dates_within_24_hours(
                        season["start_time"]["cn"], activity["start_time"]["cn"]
                    ) and dates_within_24_hours(season["end_time"]["cn"], activity["end_time"]["cn"]):
...
class IslandSeasonRequest:
    def __init__(self, item):
        ...
        self.request = (
            {request[0]: request[1] for _, request in item["request"].items()}
...
class IslandShop:
    ...
    def __init__(self, item):
        ...
        if self.tag_type == 3:
            ...
            self.parent_id = None
        self.currency = (
            [coin[2] for _, coin in item["top_resource"].items()] if isinstance(item["top_resource"], dict) else []
        )
        self.goods = (
            [shop_recipe_id for _, shop_recipe_id in item["goods_id"].items()]
            if isinstance(item["goods_id"], dict)
            else []
        )
    ...
    def __init__(self):
        ...
        for index, item in data.items():
            ...
            self.item[index] = {
                ...
                "items": {itm[1]: itm[2] for _, itm in item["items"].items()},
                "start_time": {
                    "cn": None,
                    "en": None,
                    "jp": None,
    ...
    def __init__(self):
        ...
        for index, item in data.items():
            ...
            self.item[index] = {
                ...
                "items": {item["target_item"]: item["target_num"]},
            }
...
class IslandRestaurantExtractor:
    def __init__(self):
        ...
        for index, item in data.items():
            if not isinstance(index, int):
                continue
            self.restaurant.update({item["id"]: {recipe[0]: recipe[1] for _, recipe in item["item_id"].items()}})
